...
public class RequestPanel_Nurse extends JPanel {
    ...
    private void addItem() {
        ...
        if (MedicalRequestSQL.addRequest(category, item, qty, status, currentUserId)) {
            if (Database.LogisticsSQL.insertOrderFromDoctorRequest(item, qty)) {
                System.out.println("Logistics logging status sequence successfully verified.");
            } else {
                System.out.println("Warning: Medical request recorded, but logistics pipeline entry initialization failed.");
            }
            loadDataFromDatabase();
            txtQty.setText("");
